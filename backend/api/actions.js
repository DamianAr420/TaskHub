...
class Actions {
  ...
  async editProfile(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ message: "Profil zaktualizowany", user: updatedUser });
    } catch (err) {
  ...
  async editGroup(req, res) {
    try {
      ...
      const project = await Project.findById(projectId);
      if (!project) {
        return res.status(404).json({ error: "Brak projektu" });
      }

      const group = project.groups.id(groupId);
      if (!group) {
        return res.status(404).json({ error: "Brak grupy" });
      }
      ...
      res.status(200).json({ message: "Grupa zaktualizowana", group });
    } catch (err) {
      console.error("Błąd edytowania grupy:", err);
      res.status(500).json({ error: "Błąd serwera" });
    }
  }

  // Edycja kolumny
  async editColumn(req, res) {
    try {
      const { projectId, groupId, columnId } = req.params;
      const { name } = req.body;
      if (!name) {
      ...
      const column = group.columns.id(columnId);
      if (!column) {
        return res.status(404).json({ error: "Brak kolumny" });
      }
      ...
      res.status(200).json({ message: "Kolumna zaktualizowana", column });
    } catch (err) {
      console.error("Błąd edytowania kolumny:", err);
      res.status(500).json({ error: "Błąd serwera" });
    }
  }
  ...
  async editTask(req, res) {
    try {
      ...
      if (!task) {
        return res.status(404).json({ error: "Brak zadania" });
      }
      ...
      res.status(200).json({ message: "Zadanie zaktualizowane", task });
    } catch (err) {
      console.error("Błąd edytowania zadania:", err);
      res.status(500).json({ error: "Błąd serwera" });
    }
  }
}
